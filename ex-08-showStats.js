...
function showStats(num, arr){
  ...
  for ( i = 0; i < arr.length; i++){
    if ( num === arr[i]) {
      equalTo++;
      result.countValuesInputIsEqualTo = equalTo;
    } if (num > arr[i]) {
      greaterThan++;
      result.countValuesInputIsGreaterThan = greaterThan;
    } if (num < arr[i]) {
      lessThan++;
      result.countValuesInputIsLessThan = lessThan;
    }
    result.inputArraySize = arr.length;
  }
  return result;
}
